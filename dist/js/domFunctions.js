export const setPlaceholderText = () => {
    ...
    window.innerWidth < 400 ? (input.placeholder = "City, State, Country") : (input.placeholder = "City, State, Country, Zip Code");
};
...
const updateWeatherLocationHeader = (message) =>{
    ...
    if (message.indexOf("Lat:") !== -1 && message.indexOf("Long:") !== -1){
        ...
        const lat = mapArray[0]+mapArray[1].slice(0,6)
        const lon = mapArray[2]+mapArray[3].slice(0, 6)
...
const createCurrentConditionsDivs = (weatherObj, unit) => {
    const tempUnit1 = unit === "imperial" ? "F" : "C";
    const tempUnit =createElem("div", "unit", tempUnit1);
    ...
    return [icon, temp, tempUnit, desc, feels, maxTemp, minTemp, humidity, wind];
}
...
const createDailyForecastDivs = (dayWeather) => {
    const dayAbbreviationText = getDayAbbrevation(dayWeather.dt);
